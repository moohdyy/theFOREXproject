...
public class JapaneseCandlestick {
	...
	public Types determineType() {
		if (category == Categories.None) {
			if (highestValue == lowestValue) {
				...
				&& (lowestValue == endingValue)) {
			return Types.BlackMarubozu;
		} else if ((highestValue == endingValue)
				&& (lowestValue == startingValue)) {
	...
	public static Patterns determinePattern(
			ArrayList<JapaneseCandlestick> candlesticks) {
		...
		if (i > 0) {
			if (candlesticks.get(i).type == Types.Hammer) {
				...
				} else if (candlesticks.get(i - 1).highestValue <= candlesticks
						.get(i).lowerBody) {
			...
			double dif = candlesticks.get(i).highestValue
					- candlesticks.get(i).lowestValue;
			dif *= 0.05;
			if (hasDowntrend(candlesticks, i - 1)) {
				if (candlesticks.get(i).category == Categories.Bullish) {
					if (candlesticks.get(i - 1).category == Categories.Bearish) {
						if (Math.abs(candlesticks.get(i - 1).lowerBody
								- candlesticks.get(i).lowerBody) <= Math
								.abs(dif)) {
							double downStick = candlesticks.get(i - 1).lowerBody
									- candlesticks.get(i - 1).lowestValue;
							double downStick1 = candlesticks.get(i).lowerBody
									- candlesticks.get(i).lowestValue;
				...
				if (candlesticks.get(i - 1).category == Categories.Bullish) {
					if (candlesticks.get(i).category == Categories.Bearish) {
						if (Math.abs(candlesticks.get(i - 1).upperBody
								- candlesticks.get(i).upperBody) <= Math
								.abs(dif)) {
							double upperStick = candlesticks.get(i - 1).upperBody
									- candlesticks.get(i - 1).lowestValue;
							double upperStick1 = candlesticks.get(i).upperBody
									- candlesticks.get(i).lowestValue;
		...
		if (i > 0) {
			...
			if (candlesticks.get(i - 1).type == Types.SpinningTop) {
				if (hasDowntrend(candlesticks, i - 2)) {
					if (candlesticks.get(i - 2).category == Categories.Bearish) {
						double middle = candlesticks.get(i - 2).highestValue
								- candlesticks.get(i - 2).lowestValue;
						middle /= 2;
				...
				if (hasDowntrend(candlesticks, i - 2)) {
					if (c.category == Categories.Bearish) {
						...
						if (midpointC < candlesticks.get(i - 1).endingValue) {
							if (candlesticks.get(i).endingValue > candlesticks
									.get(i - 1).highestValue) {
			...
			if (hasDowntrend(candlesticks, i - 3)) {
				if ((front.category == Categories.Bullish)
						&& (middle.category == Categories.Bullish)
						&& (back.category == Categories.Bullish)) {
					if ((back.getBodySize() <= middle.getSize())
							&& (middle.hasSmallUpperShadow())) {
				...
				if ((front.category == Categories.Bearish)
						&& (middle.category == Categories.Bearish)
						&& (back.category == Categories.Bearish)) {
					if ((back.getBodySize() <= middle.getSize())
							&& (middle.smallLowerShadow())) {
						if (middle.getBodySize() <= front.getSize()) {
	...
	public static boolean hasDowntrend(ArrayList<JapaneseCandlestick> candles,
			int i) {
		ArrayList<Double> middlepoints = new ArrayList<Double>();
		for (int j = i - 1; j >= 0; j--) {
			middlepoints
					.add((candles.get(j).highestValue - candles.get(j).lowestValue) / 2);
		}
		int j = 0;
		while ((j < middlepoints.size() - 1)
				&& (middlepoints.get(j) < middlepoints.get(j + 1))) {
			j++;
		}
		if (j >= 5) {
			return true;
	...
	public static boolean hasUptrend(ArrayList<JapaneseCandlestick> candles,
			int i) {
		ArrayList<Double> middlepoints = new ArrayList<Double>();
		for (int j = i - 1; j >= 0; j--) {
			middlepoints
